...
def run_labeler(session_id):
    ...
    while True:
        ...
        # Controls
        # ... (Display Logic remains the same above this) ...

        # Waitkey delay based on webcam FPS
        ...
        # --- NEW: TIMELINE SCRUBBING (Both Videos) ---
        elif key == ord('a'): 
            current_time_sec = max(0, current_time_sec - JUMP_SECONDS)
        elif key == ord('s'): 
            current_time_sec += JUMP_SECONDS
            
        # --- NEW: FRAME-BY-FRAME PRECISION (Use Comma and Period keys) ---
        elif not is_playing and key == ord(','): 
            # Step backward exactly 1 frame
            current_time_sec = max(0, current_time_sec - (1.0 / fps_web))
        elif not is_playing and key == ord('.'): 
            # Step forward exactly 1 frame
            current_time_sec += (1.0 / fps_web)
            
        # --- SYNC CALIBRATION (Shifts ONLY the phone video) ---
        elif key == ord('['): 
            phone_offset_sec -= 0.1  
        elif key == ord(']'): 
            phone_offset_sec += 0.1  
            
        # --- LABELING ---
        elif key == ord('c'): 
            labeled_events.append((current_timestamp, "CHEAT_PHYSICAL"))
            print(f"Logged CHEAT_PHYSICAL at {time_str} ({current_timestamp:.2f}s)")
        elif key == ord('d'): 
            labeled_events.append((current_timestamp, "CHEAT_DIGITAL"))
            print(f"Logged CHEAT_DIGITAL at {time_str} ({current_timestamp:.2f}s)")
        elif key == ord('n'): 
            labeled_events.append((current_timestamp, "NORMAL"))
            print(f"Logged NORMAL at {time_str} ({current_timestamp:.2f}s)")
            
        # --- NEW: UNDO MISTAKES ---
        elif key == ord('u'):
            if len(labeled_events) > 1: # Keep the initial SESSION_START normal label
                removed = labeled_events.pop()
                print(f"UNDO: Removed label '{removed[1]}' at {removed[0]:.2f}s")
            else:
                print("Cannot undo the starting label.")
